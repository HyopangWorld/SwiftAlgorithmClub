...
func quickSort(a: [String]) -> [String] {
    ...
    // (key값 보다 작은 배열의 정렬) + key + (key값보다 큰 배열의 정렬)
    return quickSort(a: array.filter{ !isBigger(key: key, val: $0) }) + [key] + quickSort(a: array.filter{ isBigger(key: key, val: $0) })
}

func isBigger(key: String, val: String) -> Bool {
    if key.count < val.count { return true }
    else if key.count > val.count { return false }
    
    let numbers1 = Array(key)
    let numbers2 = Array(val)
    
    for i in 0..<numbers1.count {
        if numbers1[i] < numbers2[i] { return true }
        else if numbers1[i] > numbers2[i] { return false }
    }
